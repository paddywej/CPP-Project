...
void ManageGenres::on_addButton_clicked()
{
    ...
    if (ID.isEmpty() || genreName.isEmpty()) {
        QMessageBox::warning(this, "Empty Fields", "Both ID and Genre fields must be filled.");
        return;
    }

    for (const Genre& existingGenre : genres) {
        if (existingGenre.id == ID) {
            QMessageBox::warning(this, "Duplicate ID", "A genre with the same ID already exists.");
            return;
        }
    }
    ...
    if (!selectedRows.isEmpty())
    {
        ...
        for (const Genre& existingGenre : genres) {
            if (existingGenre.id == editedID && existingGenre.id != selectedID) {
                QMessageBox::warning(this, "Duplicate ID", "An entry with the same ID already exists.");
                return;
            }
        }
